...
#[async_trait]
impl DatasetRepo for SqliteRepo {
    ...
    async fn data_points(&self, dataset_id: i64) -> Result<Vec<DataPoint>> {
        sqlx::query_as::<_, DataPoint>("SELECT * FROM data_points WHERE dataset_id = ? ORDER BY date")
            .bind(dataset_id)
            .fetch_all(&self.pool)
            .await
            .map_err(|e| RepoError::Database(e.to_string()))
...
#[async_trait]
impl TargetRepo for SqliteRepo {
    ...
    async fn get_target(&self, id: i64) -> Result<Option<PredictionTarget>> {
        sqlx::query_as::<_, PredictionTarget>("SELECT * FROM prediction_targets WHERE id = ?")
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| RepoError::Database(e.to_string()))
...
#[async_trait]
impl PredictionRepo for SqliteRepo {
...
#[async_trait]
impl DataSourceRepo for SqliteRepo {
    ...
    async fn get_source_by_name(&self, name: &str) -> Result<Option<DataSource>> {
        sqlx::query_as::<_, DataSource>("SELECT * FROM data_sources WHERE name = ?")
            .bind(name)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| RepoError::Database(e.to_string()))
    ...
    async fn create_source(&self, source: &CreateDataSource) -> Result<DataSource> {
        let row = sqlx::query(
            "INSERT INTO data_sources (name, display_name, api_base_url, api_key, description) VALUES (?, ?, ?, ?, ?)",
        )
        .bind(&source.name)
    ...
    async fn update_source(&self, id: i64, updates: &UpdateDataSource) -> Result<DataSource> {
        ...
        let enabled = updates.enabled.map(|v| v as i64);
...
pub async fn run_migrations(pool: &SqlitePool) -> Result<()> {
    ...
    sqlx::query(
        r#"ALTER TABLE datasets ADD COLUMN external_id TEXT"#,
    )
    .execute(pool)
    .await
    .ok();
